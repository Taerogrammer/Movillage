...
### MVC 패턴의 장단점에 대해 이해할 수 있었습니다.
- MVC 패턴은 초기 개발을 진행할 때에 개발 속도가 빠르다는 장점이 있지만, ViewController의 역할 가중으로 인해 코드가 비대해질 수 있다는 단점을 느꼈습니다. 
그렇기 때문에 프로젝트의 볼륨을 크게 잡는다고 하면 MVC 패턴에서 다른 패턴으로 변경하는 것이 필요하다고 생각되었습니다.

### 추상화된 모델 객체를 생성하는 것이 개발 속도를 향상시키는 방법이 될 수 있음을 느꼈습니다.
- 객체를 생성하기 위해 초반 설계 및 구현이 필요하지만, 오히려 한 번 틀을 잡아놓으면 추후에 훨씬 간편하다는 것을 느꼈습니다.

### 서드 파티 라이브러리에 대한 이해가 중요하다고 느꼈습니다.
- 대표적인 예시로 `Kingfisher`에 DownSampling과 디스크 캐싱의 기간 등에 대해 알지 못했으면 사용하지 못했을 것입니다.
즉, 라이브러리를 사용하더라도 어떠한 기능이 있고, 해당 라이브러리의 특징, 장단점 등을 파악하는 것이 중요하다고 느꼈습니다.

<br>

## 아쉬운 점 (Lacked)

### 메모리 누수에 대한 고려를 하지 못했습니다.
- 개발 기간 안에 프로젝트를 완수하기 위해 구현 위주로 개발을 진행하였기 때문에 메모리 누수에 대한 처리를 대응하지 못하였습니다.

### API에 있는 모든 예외 상황에 대한 처리를 진행하지 못했습니다.
- TMDB API에 여러 가지 예외 상황이 존재하였는데, 모든 경우에 대한 예외 처리를 수행하지 못하였습니다. 예상치 못하게 `nil` 값이 존재하는 경우가 많았습니다.
물론 강제 언래핑을 통해 앱이 종료되는 방식은 아니었으나, request error가 나타날 가능성이 있습니다.
...
### MVVM 패턴으로 리팩토링 하고자 합니다.
- 프로젝트를 진행함에 따라 비즈니스 로직과 UI 로직의 분리의 필요성을 느끼게 되어 MVVM 패턴을 기반으로 리팩토링을 진행하고자 합니다.
MVVM 패턴은 UI 로직과 비즈니스 로직을 분리함으로써 ViewController의 부담을 줄임과 동시에 로직을 명확히 구분하여 개발을 진행할 수 있다는 장점이 존재합니다.


### 클로저를 사용하면서 발생할 수 있는 메모리 누수 이슈를 해결하고자 합니다.
- 클로저를 사용했을 때 발생하는 강한 참조에 대한 이슈를 대응하고, 추가적인 메모리 누수 이슈를 해결하고자 합니다.
메모리 누수에 대한 확인은 `deinit`메서드를 기반으로 확인하고, 클로저 구문이 있는 부분에 weak self를 선언하는 등의 방법으로 해결하고자 합니다.

### API에 있는 모든 예외 상황을 처리하고자 합니다.
- 모든 값이 `nil`이 존재할 수도 있다는 가정을 기반으로 NetworkManager를 수정하고자 합니다. 또한 정보가 존재하지 않을 때에 대한 UI 처리를 진행하고자 합니다.

### 다양한 API를 통해 더 많은 기능을 추가하고자 합니다.
- TMDB API는 다양한 기능을 제공하기 때문에, 이를 기반으로 기능을 추가하고, 더 나아가 다른 API 또한 사용하고자 합니다.